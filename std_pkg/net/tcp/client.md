...
- context 的使用
    ...
    - 通常在入口处做一次检查，如果 ctx.Done() 直接返回。
    - 如果操作分成几个重要的 step，可以在每个 step 成功后，判断一次上下文是否结束。如果结束，则返回 ctx.Err() 并释放相关资源。
    - 在一些类似重复操作中，比如 for 循环，在每一个 step 成功后，也可以判断一次。
    - 还可以专门拉起一个监控协程，监控 ctx.Done()。 当 ctx.Done() 的时候，与主协程直接进行其他的同步。
    
    总之，就两种方式：同步、异步。同步通常在任务入口、任务各个 step 成功等一些关键节点进行检查。异步就是开启一个专门的监控协程。